...
from src.protbert import Baseline, MLP, BaselineOne
from src.baseline_dataset import SAbDabDataset, SMOTEDataset
from src.metrics import MCC
from src.data_loading_split import load_data
from src.training_eval import eval_model, model_initializer, embedding_phase
...
from captum.attr import LayerConductance, LayerIntegratedGradients, IntegratedGradients, TokenReferenceBase, visualization
...
def interpret_sequence(model, sequence, min_len = None, label = 0):
  tokens = tokenizer(sequence['VH'])
  ...
  add_attributions_to_visualizer(attributions_ig, sequence, pred[pred_ind], pred_ind, label, delta, vis_data_records_ig)
...
def add_attributions_to_visualizer(attributions, sequence, pred, pred_ind, label, delta, vis_data_records):
  ...
  # storing couple samples in an array for visualization purpuses
...
if __name__ == "__main__":
  ...
  if args.smote:
    if args.model == 'antiberty':
      dataset = SMOTEDataset('/disk1/abtarget/dataset/sabdab/split/sabdab_200423_test_norep_antiberty_embeddings.csv')
    elif args.model == 'protbert':
      dataset = SMOTEDataset('/disk1/abtarget/dataset/sabdab/split/sabdab_200423_test_norep_protbert_embeddings.csv')
    else:
      exit()
  else:
    dataset = SAbDabDataset('/disk1/abtarget/dataset/sabdab/split/sabdab_200423_test_norep.csv')

  target_classes = ["Protein", "Non-protein"]
  ...
  if args.smote:
    ...
    model_one = BaselineOne(args.batch_size, device, nn_classes=args.n_class, freeze_bert=args.pretrain, model_name=args.model)
  ...
  for count, inputs in enumerate(dataloaders["test"]):
    ...
    interpret_sequence(model, inputs, label = labels)
